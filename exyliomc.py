...
CANAL_LOGS_OBJ = None
CANAL_ARCHIVOS_OBJ = None  # Nuevo: Canal para archivos adjuntos

# Carpeta base para adjuntos
ATTACHMENTS_BASE = "ticket_attachments"
...
def cargar_datos():
    ...
    return tickets_data, config_data

# Guardar datos en archivos JSON
def guardar_datos(tickets_data, staff_roles=None):
    # Guardar tickets
    with open(TICKETS_FILE, 'w') as f:
        json.dump(tickets_data, f, indent=4)
    
    # Guardar roles de staff si se proporcionan
...
class EstadoSelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="Cambiar estado del ticket...", options=options, custom_id="estado_select")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if canal_id in tickets:
            tickets[canal_id]["estado"] = nuevo_estado
            guardar_datos(tickets, ROLES_STAFF)
...
@bot.event
async def on_ready():
    global CANAL_LOGS_OBJ, CANAL_ARCHIVOS_OBJ, tickets, config
    ...
    for canal_id in list(tickets.keys()):
        canal = bot.get_channel(int(canal_id))
        if not canal:  # El canal ya no existe
            tickets_a_eliminar.append(canal_id)
    
    for canal_id in tickets_a_eliminar:
        del tickets[canal_id]
    
    if tickets_a_eliminar:
        guardar_datos(tickets, ROLES_STAFF)
        print(f"♻️ Se limpiaron {len(tickets_a_eliminar)} tickets de canales eliminados")
    ...
    for canal_id, ticket_data in tickets.items():
        if not ticket_data.get("fecha_archivado"):
            canal = bot.get_channel(int(canal_id))
            if canal:
                # Verificar permisos
...
async def limpieza_periodica():
    ...
    while not bot.is_closed():
        ...
        limpiar_adjuntos_antiguos()
        print("♻️ Ejecutada limpieza periódica de archivos adjuntos")

# COMANDO EN ESPAÑOL CORREGIDO
...
async def crear_ticket(payload):
    ...
    # Verificación robusta de tickets existentes
    for ticket_id, ticket_data in list(tickets.items()):
        # Verificar si el canal existe realmente
        canal_existente = guild.get_channel(int(ticket_id))
        if not canal_existente:
            continue  # El canal fue borrado, ignorar
            
        # Verificar si el ticket pertenece al usuario y está activo
        if (ticket_data["usuario_id"] == str(usuario.id) and 
            not ticket_data.get("fecha_archivado") and 
            ticket_data.get("guild_id") == str(guild.id)):
            
            try:
                canal = guild.get_channel(int(ticket_id))
                embed = discord.Embed(
    ...
    try:
        ...
        # Manejar error sin intentar registrar log (para evitar bucle)
...
async def transcript_handler(interaction: discord.Interaction):
    ...
    try:
        ...
        messages = []
        # Recopilar mensajes y descargar adjuntos
        ...
        embed.add_field(
            name="Detalles",
            value=f"• Mensajes registrados: {len(messages)}\n• Canal: {interaction.channel.mention}",
            inline=False
        )
...
async def listar_tickets(interaction: discord.Interaction):
    tickets_activos = [t for t in tickets.values() if not t.get("fecha_archivado")]
    ...
    for canal_id, ticket in tickets.items():
        if not ticket.get("fecha_archivado"):
...
async def estadisticas_tickets(interaction: discord.Interaction):
    activos = sum(1 for t in tickets.values() if not t.get("fecha_archivado"))
    archivados = len(tickets) - activos
    ...
    if archivados > 0:
        ...
        for t in tickets.values():
            if t.get("fecha_archivado"):
                creado = datetime.datetime.fromisoformat(t["fecha_creacion"])
                archivado = datetime.datetime.fromisoformat(t["fecha_archivado"])
                total_segundos += (archivado - creado).total_seconds()
        
        promedio = total_segundos / archivados
        horas = int(promedio // 3600)
        minutos = int((promedio % 3600) // 60)
        embed.add_field(
...
async def buscar_tickets(
    ...
    # Filtrar tickets válidos (canales existentes)
    resultados = []
    for canal_id, ticket in tickets.items():
        canal = interaction.guild.get_channel(int(canal_id))
        if not canal:
            continue  # Canal eliminado, se ignorará
            
        match_usuario = usuario is None or ticket["usuario_id"] == str(usuario.id)
        match_estado = estado is None or ticket["estado"] == estado
        
        if match_usuario and match_estado:
            user = interaction.guild.get_member(int(ticket["usuario_id"]))
            user_name = user.display_name if user else f"ID:{ticket['usuario_id']}"
            resultados.append(f"{ticket['estado']} - {canal.mention} - {user_name}")
    ...
    if resultados:
        embed.description = "\n".join(resultados[:8])  # Limitar a 8 resultados
        if len(resultados) > 8:
